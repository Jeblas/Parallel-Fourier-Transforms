...
int main(int argc, char * argv[]) {
	...
	memcpy(input_image, image_handler.get_image_data(),
		   sizeof(Complex) * image_handler.get_width() * image_handler.get_height());
	...
	Complex *out2_image_inverse = (Complex *) malloc(
			sizeof(Complex) * image_handler.get_width() * image_handler.get_height());

//	single_thread_naive(input_image, out_image, image_handler);
	cuda_ufft(input_image, out2_image, image_handler);
	cuda_naive(input_image, out_image, image_handler, FORWARD);
//	cuda_naive(out2_image, out2_image_inverse, image_handler, REVERSE);
...
//			std::cout << out2_image[i* image_handler.get_width() + j] << ", ";
//		}
//		std::cout << std::endl;
//	}
//	for(int i = 0; i < image_handler.get_height(); i ++){
